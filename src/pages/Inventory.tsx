...
import { Search, Plus, FileSpreadsheet, Edit2, FolderPlus } from 'lucide-react';
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 bg-white border rounded-lg hover:bg-gray-50">
        ...
        <div className="flex-1 overflow-auto p-0">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider border-b">Asset Details</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider border-b">Category</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider border-b">Status</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider border-b">Updated</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-12 text-center text-gray-500">
                    ...
                    <p className="text-sm">Select a different category or add a new asset.</p>
                </td></tr>
              ) : (
                assets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-900">ID: {asset.id.slice(0, 8)}...</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">{asset.subCategory?.name || 'Unknown'}</td>
                    <td className="px-6 py-4">
